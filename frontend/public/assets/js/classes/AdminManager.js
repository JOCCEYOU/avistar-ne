            ...
            <div class="admin-user-row" style="display: flex; align-items: center; justify-content: space-between; padding: 1rem; border-bottom: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); margin-bottom: 0.5rem; border-radius: 8px;">
                ...
                <div class="admin-user-actions" style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; justify-content: flex-end; margin-top: 0;">
                    ...
                    ${u.role !== 'admin' ? `
                        ...
                        <button onclick="window.modalManager.showConfirm('Confirmar Acción', '¿Eliminar usuario permanentemente?', 'Eliminar', 'Cancelar', true, () => window.adminManager.deleteUser(${u.id}))" class="btn btn-outline" style="border-color: #ef4444; color: #ef4444; font-size: 0.75rem; padding: 0.3rem 0.6rem;" title="Eliminar Usuario">🗑️ Eliminar</button>
                        <button onclick="window.modalManager.showConfirm('Confirmar Acción', '¿Hacer Administrador a este usuario?', 'Aceptar', 'Cancelar', false, () => window.adminManager.makeAdmin(${u.id}))" class="btn btn-outline" style="border-color: #3b82f6; color: #3b82f6; font-size: 0.75rem; padding: 0.3rem 0.6rem;" title="Hacer Admin">🛡️ Crear Admin</button>
                    ` : ''}
                    ${u.role === 'admin' && u.id !== 1 ? `
                        <button onclick="window.modalManager.showConfirm('Confirmar Acción', '¿Eliminar permisos de Administrador?', 'Revocar', 'Cancelar', true, () => window.adminManager.revokeAdmin(${u.id}))" class="btn btn-outline" style="border-color: #ef4444; color: #ef4444; font-size: 0.75rem; padding: 0.3rem 0.6rem;" title="Revocar Admin">❌ Revocar Permisos</button>
                    ` : ''}
                </div>
            </div>
        `;
